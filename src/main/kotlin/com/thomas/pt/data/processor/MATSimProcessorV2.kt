...
/**
 * Optimized MATSim processor using DuckDB for heavy aggregations.
 * - Uses read_arrow() for direct Arrow IPC file reading
 * - Uses GENERATE_SERIES for time slicing (replaces row-by-row sliceByHour)
 * - Uses MAP() for nested map building in SQL
 * - EWT interpolation remains in Kotlin (requires metadata.busRoutes)
 */
class MATSimProcessorV2(configPath: Path) : AutoCloseable {
    private val logger = LoggerFactory.getLogger(javaClass)
    private val db = DuckDBHelper()
    ...
    private val lineFilter: String  // SQL WHERE clause fragment
    ...
    private inner class ProcessAvgLoadFactor {
        fun process(): AnyFrame
            = db.query("""
                WITH bus_data AS (
                    SELECT link_id, line_id, passenger_load,
                ...
                SELECT lf.link_id, lf.avg_lf, lm.line_avg_lf
                FROM link_lf lf
    ...
    private inner class ProcessVehicleFlow {
        fun process(): AnyFrame
            = db.query("""
                WITH base AS (
                    SELECT *, (exit_time - enter_time) as duration
                    FROM read_arrow('$linkRecordsPath')
                ...
                sliced AS (
                    ...
                    LATERAL (
                        SELECT UNNEST(GENERATE_SERIES(
                            CAST(enter_time / 3600 AS INTEGER),
                            CAST((exit_time - 1) / 3600 AS INTEGER)
                        )) as hour
                    )
                ),
                hourly AS (
    ...
    fun processAll() {
        val time = measureTime {
            ...
            logger.info("Average trip length: {} meters", "%.2f".format(avgTripLength))
